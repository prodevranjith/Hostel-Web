...
require('dotenv').config();  // Loads environment variables from Vercel
...
// Nodemailer Transporter Setup using Vercel environment variables
const transporter = nodemailer.createTransport({
  service: 'gmail',
  auth: {
...
app.post('/send-message', async (req, res) => {
  ...
  const mailOptions = {
    from: email,
    to: process.env.RECEIVER_EMAIL,  // Recipient email address from environment variables
    subject: `New message from ${name}`,
    text: `You have received a new message from the contact form:
